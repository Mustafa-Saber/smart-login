...
let mess =
  (message.innerHTML = `<span class="text-danger m-3 fa-2x fw-bold">do you have an account?</span>`);
localStorage.getItem("accounts") == null
  ? mess
  : (loginUser = JSON.parse(localStorage.getItem("accounts")));
...
function checkLoginEmpty() {
  let regxLogin = function () {
    let regxe = /^\S/;
    if (
      regxe.test(signinEmail.value) == true ||
      regxe.test(signinPassword.value) == true
    ) {
      return true;
    } else {
      return false;
...
function checkForlogin() {
  for (let n in loginUser) {
    if (checkLoginEmpty() == true) {
      ...
      signinEmail.value.toLowerCase() == loginUser[n].email.toLowerCase() &&
      signinPassword.value.toLowerCase() == loginUser[n].password.toLowerCase()
    ) {
      console.log(11);
...
let href = () => (window.location.href = "index.html");
